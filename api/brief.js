...
"financials" (array of up to 5 fiscal years, newest first. For EACH year provide these fields: "year" (e.g. "FY2025"); "revenue"; "profit" (net profit); "netassets" (total shareholders' equity / net assets for THAT fiscal year, e.g. "S$120m" — this is a headline figure in every annual report, so provide it for every year you can find); "navps" (NAV / book value per share for that year if it is directly reported; otherwise null — the app will calculate it from netassets / shares); "dividend" (total dividend per share declared for that year, e.g. "S$0.08"); "roe" (return on equity as a percent like "12%" if directly reported, else null — the app can calculate it). Net assets per year is the key input — fill it for every year possible. NEVER guess or fabricate figures — an honest blank is better than a wrong number),
